...
export function buildSystemPrompt(): string {
  ...
  return [
    ...
    "- Asked for a whole subject on this canvas — 'make a bush', 'draw a health potion', 'replace this with a stone wall' — call draw_from_prompt. Never hand-draw a whole subject pixel by pixel: it burns your entire turn budget and the result is a shapeless blob. Hand-drawing is for changes you can name in a few words.",
    ...
    "- Use draw_from_prompt to draw the open canvas from a description, generate_variation_set for a family, derive_variant for one directed variation, and inpaint_region for a requested local repair. All are slow paid model calls — tens of seconds each, one image per call: never call them to brainstorm, never call one twice for the same request, and never call them unless the human explicitly asked to create or change the art. Say what you are about to generate before you call one, so the wait is not silent.",
